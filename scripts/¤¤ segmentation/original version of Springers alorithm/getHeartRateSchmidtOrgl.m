...
function [heartRate, systolicTimeInterval,signal_autocorrelation] = getHeartRateSchmidtOrgl(audio_data, Fs, figures)
...
%     figure('Name', 'Heart rate calculation figure');
    ...
    hold on;
    plot(true_index, signal_autocorrelation(floor(true_index)),'ro');
    plot((min_sys_duration+pos), signal_autocorrelation((min_sys_duration+pos)), 'mo');
    xline([min_index,max_index])
%     xlabel('Samples');
