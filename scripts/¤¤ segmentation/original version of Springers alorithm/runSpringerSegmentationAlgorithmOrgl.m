...
function [assigned_states, heartPar, NLL] = runSpringerSegmentationAlgorithmOrgl(audio_data, Fs, HMMpar, heartPar, figures)
...
if isempty(heartPar)
    [heartPar.rate, heartPar.sysDuration] = getHeartRateSchmidtOrgl(audio_data, Fs);
end

[delta, ~, qt] = viterbiDecodePCG_SpringerMod(PCG_Features, HMMpar, heartPar, featuresFs);
NLL = -max(delta(end,:));
