...
IposCases = data.(murSt)>=2;
balance = false;
Ncomp = [30,200];
[Xval,Yval] = genTrainOrValSet(data,N,aa,IposCases,balance,segLines,Ncomp);
%% predict on validation set:
Ypred = classify(net,Xval,'MiniBatchSize',miniBatchSize);
%% see performance curve:
clearvars T p X Y AUC
figure()
Nloc = 1;
Nval = height(data);
[AUC,X,Y,T,p] = performanceSummaryNeurNet(Xval,Yval,net,Nval,NsegPerPCG,true,Nloc);
...
% curve:
[~,Ioptimal] = max( (1-X.whole + Y.whole).*(Y.whole>=0.85))
opt_x_coord = .9; %get x-coordinate of a good point on performance curve
[~,imin] = min(abs(Y.whole - opt_x_coord))  
peakPerformance = [Y.whole(Ioptimal),1-X.whole(Ioptimal)]
u = T.whole(imin)
%% Make predictions with new prediction threshold:
Ypred = categorical(double(p.murWhole > u*(1 - .1)));
Yval = mode(reshapeSegmentScores(Yval,Nval,NsegPerPCG,Nloc),2);
% compute accuracy:
sum(Ypred==Yval)/212
% sensitivity and specificity:
[condProb(Ypred=='1',Yval=='1'),...
condProb(Ypred=='0',Yval=='0')]
%%
% sensitivity and specificity for detecting VHD using murmur prediction:
VHDthr = 4;
Isick = data.MRGRADE_T72>=VHDthr;
[condProb(Ypred=='1',Isick),...
condProb(Ypred=='0',~Isick),...
% sensitivity and specificity using murmur annotations:
% ARthr = 1;
condProb(Yval=='1',Isick),...
condProb(Yval=='0',~Isick)]
sum(Isick)
%% whole segments:
dataVal = HSdataVal;
clearvars I N
...
% indicating that perhaps the algorithm is capable of picking up on murmurs
