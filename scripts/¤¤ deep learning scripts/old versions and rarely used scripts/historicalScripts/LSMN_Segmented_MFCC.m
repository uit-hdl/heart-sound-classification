...
options = trainingOptions('adam', ...
    ...
    'OutputFcn',@(info)myCostumOutputFcn(info,2*3600));
%% Train network
net = trainNetwork(Xtrain,Ytrain,layers,options);
% save('trained networks/netAA1murG2_v3.mat','net')
% load netAA1234murG2_v2
% load netAA1murG2_v1
%% predict
Ypred = classify(net,Xval,'MiniBatchSize',miniBatchSize);
figure()
[AUC,X,Y,T] = performanceSummaryNeurNet(Xval,Yval,net,Nval,NsegPerPCG);
